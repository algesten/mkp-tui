...
/// **The `GetQueueSince` reply is dropped on the floor.**
///
/// `235d06b` added the request, and round 3's test asserts only that
/// it goes out. The server answers it with
/// `Response { seq: req.seq, msg: ServerMsg::QueueCatchUp { .. } }`
/// (`server/src/session.rs:365`) — a *reply*, not a broadcast. Only
/// `fold_broadcast` (seq == 0) knows how to apply a `QueueCatchUp`;
/// `mirror_response_into_source` has no arm for it, so the frame falls
/// through to `sources.responses`, where nothing ever reads it. The
/// resync is inert.
///
/// The mock here is a server that has nothing new to volunteer on
/// reconnect, so `GetQueueSince` is the only channel by which the
/// missed delta can arrive — which is precisely the scenario the
/// request was added for.
///
/// Not a regression against `main` (nothing constructed
/// `GetQueueSince` there, and `main` does not reconnect at all): this
/// is the round-3 fix not doing what it claims.
#[test]
fn the_queue_catch_up_reply_is_applied_to_the_mirror() {
    let _ = env_logger::builder().is_test(true).try_init();
    ...
    let script: Script = {
        ...
        Box::new(move |msg| match msg {
            ...
            // The answer the real server gives: a reply on the
            // request's own seq.
            ClientMsg::GetQueueSince { queue_id, .. } => {
                vec![ScriptStep::Reply(ServerMsg::QueueCatchUp {
                    queue_id: *queue_id,
                    deltas: vec![(
                        4,
                        QueueDelta::Insert {
    ...
    assert!(
        ...
        "fixture: the reconnect should have asked for the gap"
    );
    assert_eq!(
        queue_ids(&harness.rt),
        ["alpha", "bravo", "charlie"],
        "the server answered GetQueueSince with the delta the client \
         missed and the client threw it away: a QueueCatchUp arriving \
         on a request seq reaches `mirror_response_into_source`, which \
         has no arm for it, and lands in `sources.responses` unread"
    );
    assert_eq!(
        harness.rt.sources.queue.version, 4,
        "the sync cursor was not advanced by the catch-up"
    );
}
