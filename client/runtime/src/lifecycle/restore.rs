...
//! Load / Apply(view) / OpenFirst(id) / Noop. The trampoline writes
//! `auto_restored_view = true` synchronously on Apply / OpenFirst,
//! and inserts the LoadKey into `loads_in_flight` synchronously on
//! Load — both intent writes that flip the next tick's action to
//! Noop.
